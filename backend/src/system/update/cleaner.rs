...
pub async fn cleanup_old_saturn_images(docker: Arc<bollard::Docker>) -> (usize, i64) {
    ...
    if let Ok(containers) = docker.list_containers(Some(list_c_opts)).await {
        for c in containers {
            ...
            let is_saturn = names.iter().any(|n| {
                let clean = n.trim_start_matches('/').to_lowercase();
                clean == "saturn"
                    || clean.contains("saturn-dashboard")
                    || clean.starts_with("saturn")
            });
            let state = c
                .state
                .map(|s| s.to_string())
                .unwrap_or_default()
                .to_lowercase();
            if is_saturn && (state == "created" || state == "exited" || state == "dead") {
                if let Some(id) = c.id {
                    tracing::info!("Removendo container inativo/órfão do Saturn: {}", id);
    ...
    // 2. Identify active Saturn image ID
    let mut current_saturn_image_id = None;
    if let Ok(hostname) = std::env::var("HOSTNAME") {
        ...
        if !clean_host.is_empty() {
            if let Ok(ins) = docker
                ...
                .await
            {
                current_saturn_image_id = ins.image;
            }
        }
    }
    if current_saturn_image_id.is_none() {
        for cname in &["saturn", "saturn-dashboard", "saturn-dashboard", "saturn", "Saturn", "saturn_dashboard"] {
            if let Ok(ins) = docker
                .inspect_container(
                    cname,
                    None::<bollard::query_parameters::InspectContainerOptions>,
                ...
                .await
            {
                if let Some(img_id) = ins.image {
                    current_saturn_image_id = Some(img_id);
                    break;
                }
            }
        }
    ...
    // 3. Search and remove old unused Saturn/Saturn images
    if let Some(ref current_img) = current_saturn_image_id {
        let mut list_filters = std::collections::HashMap::new();
        list_filters.insert(
            "reference".to_string(),
            vec![
                "ghcr.io/andrevictor20/saturn*".to_string(),
                "victorandre280/saturn*".to_string(),
                "ghcr.io/andrevictor20/saturn-dashboard*".to_string(),
                "victorandre280/saturn-dashboard*".to_string(),
            ],
        );
        ...
        if let Ok(images) = docker.list_images(Some(list_opts)).await {
            for img in images {
                ...
                {
                    tracing::info!("Removendo imagem antiga não utilizada do Saturn: {}", img_id);
    ...
    if deleted_count > 0 {
        tracing::info!(
            "Limpeza automática pós-atualização: {} imagem(ns) antiga(s) do Saturn removida(s), {} bytes liberados.",
            deleted_count,
            space_reclaimed
        );
    }
...
pub async fn cleanup_old_saturn_images_handler(
    State(state): State<AppState>,
) -> impl IntoResponse {
    let (deleted, space) = cleanup_old_saturn_images(state.docker.clone()).await;
    (
        StatusCode::OK,
