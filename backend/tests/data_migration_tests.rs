...
fn create_test_temp_dir() -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!("saturn_migrator_test_{}", uuid::Uuid::new_v4()));
...
fn test_migrate_copies_all_config_files_when_auth_missing() {
    ...
    fs::write(candidate.join("saturn_auth.json"), r#"{"username":"admin","hash":"$argon2id$test"}"#).unwrap();
    ...
    assert!(migrated, "Migration should succeed when target has no saturn_auth.json");

    // Verify all files were copied to target
    assert_eq!(
        fs::read_to_string(target.join("saturn_auth.json")).unwrap(),
...
fn test_migrate_does_not_overwrite_when_target_has_auth() {
    ...
    fs::write(candidate.join("saturn_auth.json"), r#"{"username":"old_admin","hash":"old_hash"}"#).unwrap();
    fs::write(candidate.join("custom_links.json"), r#"{"c1":"old_link"}"#).unwrap();

    // Target already has existing data
    fs::write(target.join("saturn_auth.json"), r#"{"username":"current_admin","hash":"current_hash"}"#).unwrap();
    fs::write(target.join("custom_links.json"), r#"{"c1":"current_link"}"#).unwrap();

    let migrated = backend::system::data_migrator::scan_and_migrate_from_dir(&candidate, &target);
    assert!(!migrated, "Migration must NOT run if target already contains saturn_auth.json");
...
fn test_migrate_handles_subdirectories_like_samba() {
    ...
    fs::write(candidate.join("saturn_auth.json"), r#"{"username":"admin","hash":"hash"}"#).unwrap();
