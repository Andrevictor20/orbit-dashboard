    ...
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-48 rounded-2xl border border-border/80 bg-card/90 backdrop-blur-3xl shadow-2xl py-1.5 z-50 animate-in fade-in zoom-in-95 duration-200 origin-top-right">
          {options.map((opt) => (
            <button key={opt.value} onClick={() => { onChange(opt.value); setIsOpen(false); }} className={`w-full text-left px-3.5 py-2 text-xs font-medium hover:bg-orbit-500/15 hover:text-orbit-400 transition-colors ${value === opt.value ? 'text-orbit-500 bg-orbit-500/10 font-semibold' : 'text-secondary hover:text-primary'}`}>
              {opt.label}
            </button>
          ))}
        </div>
        ...
        <div className="h-14 border-b border-border/60 flex items-center justify-between px-4 bg-card/35 backdrop-blur-3xl">
          <div className="flex items-center gap-2.5 overflow-hidden">
            ...
            <div className={`flex flex-col ${(isSidebarOpen || isMobileMenuOpen) ? 'block' : 'hidden md:hidden'}`}>
              <span className="text-sm font-bold tracking-tight text-primary leading-tight truncate">{settings.server_name || 'Orbit'}</span>
      ...
      <main className={`relative z-10 flex-1 flex flex-col min-h-screen transition-all duration-300 w-full min-w-0 ${isSidebarOpen ? 'md:ml-64' : 'md:ml-16'}`}>
        <header className="h-14 border-b border-border/60 flex items-center justify-between px-3 sm:px-6 sticky top-0 z-30 bg-card/45 backdrop-blur-3xl saturate-[190%] shadow-sm">
          <div className="flex items-center gap-2.5 md:hidden">
            <button onClick={() => setIsMobileMenuOpen(true)} className="w-9 h-9 flex items-center justify-center text-secondary hover:text-primary rounded-xl border border-border/70 bg-card/50 hover:bg-card/85 backdrop-blur-2xl transition-all duration-200 active:scale-95 shadow-sm focus-visible:ring-2 focus-visible:ring-orbit-500" aria-label={t('sidebar.open_navigation_menu', 'Abrir menu de navegação')}>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5 text-sm font-medium">
            <button
              ...
              aria-label={t('system.orbit_updates', 'Atualizações do Orbit')}
            >
