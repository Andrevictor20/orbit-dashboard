...
import { X, Globe, Server, Box, ShieldCheck, ShieldAlert, ExternalLink, Loader2, Check, ChevronDown, ChevronUp } from 'lucide-react';
import toast from 'react-hot-toast';
import type { IngressRule, CreateRouteResponse } from '../../types/cloudflare';
import { detectBaseDomain, parseContainerList, type ContainerOption } from './cloudflareRouteHelpers';
...
export { detectBaseDomain };

export function CloudflareAddRouteModal({ isOpen, onClose, onRouteCreated, tunnelId, existingRules }: CloudflareAddRouteModalProps) {
  ...
  useEffect(() => {
    if (!isOpen) return;
    setBaseDomain(detectBaseDomain(existingRules));
    setSubdomain(''); setRawHostname(''); setPath(''); setNoTlsVerify(false);
    setSelectedContainer(''); setSelectedPort(''); setCustomService('http://'); setAuthError(null);
    setLoadingContainers(true);
    fetch('/api/docker/containers', { headers: getAuthHeaders(), credentials: 'include' })
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data)) {
          const list = parseContainerList(data);
          setContainers(list);
          if (list.length > 0) {
            const first = list[0];
            setSelectedContainer(first.name);
            setSubdomain(first.name.toLowerCase().replace(/[^a-z0-9-]/g, '-'));
            if (first.ports.length > 0) setSelectedPort(first.ports[0].toString());
          }
        }
      })
      .catch(() => {})
  ...
  const computeFinalHostname = (): string => {
    if (hostnameMode === 'raw') return rawHostname.trim().toLowerCase().replace(/^https?:\/\//, '');
    const cleanSub = subdomain.trim().toLowerCase().replace(/\.+$/, '');
    const cleanBase = baseDomain.trim().toLowerCase().replace(/^\.+/, '');
    return cleanSub ? `${cleanSub}.${cleanBase}` : cleanBase;
  };

  const computeFinalService = (): string => {
    if (serviceMode === 'custom') return customService.trim();
    return `http://${selectedContainer}:${selectedPort.trim() || '80'}`;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/cloudflare/routes', {
        ...
        body: JSON.stringify({ hostname: cleanHostname, service: finalService, path: path.trim() || undefined, no_tls_verify: noTlsVerify || undefined }),
      });
      ...
      if (res.ok && data.success) {
        ...
        if (data.dns_message) toast(data.dns_message, { icon: 'ℹ️', duration: 6000 });
        onRouteCreated(data.route);
        onClose();
      } else {
        const errorMsg = (data as any).error || t('cloudflare.route_create_error', 'Falha ao criar rota');
        const isAuthErr = errorMsg.toLowerCase().includes('authorized') || errorMsg.toLowerCase().includes('permission') || res.status === 401 || res.status === 403;
        if (isAuthErr) { setAuthError(errorMsg); toast.error(errorMsg, { duration: 8000 }); }
        else toast.error(errorMsg);
      }
    } catch { toast.error(t('cloudflare.route_create_error', 'Falha na comunicação com o servidor')); }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-lg rounded-2xl border border-border/80 bg-card p-6 shadow-2xl backdrop-blur-xl">
        <div className="flex items-center justify-between pb-4 border-b border-border/60">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg border border-border/60 hover:bg-accent text-secondary hover:text-primary transition-colors"><X className="w-4 h-4" /></button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="text-xs font-semibold text-primary">{t('cloudflare.public_hostname', 'Hostname Público')} <span className="text-rose-500">*</span></label>
              <div className="flex bg-accent/40 border border-border/60 rounded-lg p-0.5 text-[11px]">
                {(['builder', 'raw'] as const).map(mode => (
                  <button key={mode} type="button" onClick={() => setHostnameMode(mode)} className={`px-2 py-0.5 rounded-md font-medium transition-all ${hostnameMode === mode ? 'bg-card text-primary shadow-sm font-bold' : 'text-secondary hover:text-primary'}`}>
                    {mode === 'builder' ? t('cloudflare.hostname_mode_builder', 'Construtor') : t('cloudflare.hostname_mode_raw', 'Manual')}
                  </button>
                ))}
              </div>
            </div>
            {hostnameMode === 'builder' ? (
              <div className="space-y-2">
                ...
                <div className="px-3 py-2 rounded-xl bg-accent/20 border border-border/50 text-[11px] font-mono flex items-center justify-between">
                  <span className="text-secondary/70">{t('cloudflare.final_public_url', 'URL Pública Final:')}</span>
                  <span className="font-bold text-emerald-500 dark:text-emerald-400">https://{computeFinalHostname() || '...'}</span>
                </div>
              </div>
            ) : (
          ...
          {/* Service Mode */}
          <div>
            <label className="block text-xs font-semibold text-primary mb-1.5">{t('cloudflare.target_service', 'Serviço de Destino Interno')} <span className="text-rose-500">*</span></label>
            <div className="grid grid-cols-2 gap-1.5 p-1 rounded-xl bg-accent/30 border border-border/60 mb-2.5">
              {([['container', Box, t('cloudflare.mode_container', 'Contêiner Docker')], ['custom', Server, t('cloudflare.mode_custom', 'URL Manual')]] as const).map(([mode, Icon, label]) => (
                <button key={mode} type="button" onClick={() => setServiceMode(mode)} className={`flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-xs font-medium transition-all ${serviceMode === mode ? 'bg-card text-primary shadow-sm font-semibold' : 'text-secondary hover:text-primary'}`}>
            ...
            {serviceMode === 'container' ? (
              <div className="space-y-2">
                {loadingContainers ? (
                  <div className="p-3 text-center text-xs text-secondary flex items-center justify-center gap-2"><Loader2 className="w-3.5 h-3.5 animate-spin" /><span>{t('cloudflare.loading_containers', 'Buscando contêineres...')}</span></div>
                ) : containers.length === 0 ? (
                  <div className="p-2.5 rounded-xl border border-border/60 bg-accent/20 text-xs text-secondary">{t('cloudflare.no_containers_found', 'Nenhum contêiner ativo encontrado. Use a URL Manual.')}</div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <div>
                ...
                <p className="text-[11px] text-secondary/70 font-mono">{t('cloudflare.target_preview', 'Destino configurado')}: <span className="text-primary font-bold">{computeFinalService() || '—'}</span></p>
              </div>
            ) : (
              <div>
          ...
          {/* Advanced */}
          <div className="pt-1">
            <button type="button" onClick={() => setShowAdvanced(!showAdvanced)} className="flex items-center gap-1.5 text-xs font-semibold text-secondary hover:text-primary transition-colors">
          ...
          <div className="p-3 rounded-xl border border-sky-500/20 bg-sky-500/5 text-xs text-secondary flex items-start gap-2.5">
            ...
            <div className="text-[11px] leading-relaxed">
              <span className="font-semibold text-primary block">{t('cloudflare.dns_tip_title', 'Dica de Conectividade DNS')}</span>
              {t('cloudflare.dns_tip_desc', 'Se você utiliza Wildcard DNS (*.{{domain}}), esta rota funcionará instantaneamente.', { domain: baseDomain || 'rasppi.cloud' })}{' '}
            ...
            <div className="p-3.5 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-300 text-xs space-y-2 animate-fade-in">
              <div className="flex items-start gap-2.5">
                ...
                <div className="space-y-1">
                  <p className="font-semibold text-rose-200">{t('cloudflare.route_auth_error_title', 'Permissão Insuficiente no Cloudflare')}</p>
                  <p className="text-secondary leading-relaxed">{t('cloudflare.route_auth_error_desc', "Seu API Token precisa da permissão 'Account > Cloudflare Tunnel > Edit'.")}</p>
                  <a href="https://dash.cloudflare.com/profile/api-tokens" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-rose-500/20 hover:bg-rose-500/30 border border-rose-500/40 text-rose-200 text-[11px] font-medium transition-colors">
                    <span>{t('cloudflare.route_auth_error_link', 'Abrir Tokens da Cloudflare')}</span><ExternalLink className="w-3.5 h-3.5" />
                  </a>
                </div>
              </div>
            </div>
