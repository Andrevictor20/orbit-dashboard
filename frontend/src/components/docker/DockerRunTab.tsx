      ...
      <div>
        <label className="block text-xs font-semibold text-secondary uppercase tracking-wider mb-1.5 flex items-center justify-between">
          <span className="flex items-center gap-1.5">
            <Terminal className="w-3.5 h-3.5 text-saturn-500" />
            <span>{t('docker_install.input_label', 'Comando Docker Run ou Conteúdo Compose:')}</span>
          </span>
          {parsing && (
            <span className="flex items-center gap-1 text-[11px] text-saturn-400">
        ...
        <textarea
          ...
          className="w-full bg-accent/40 border border-border rounded-xl p-3 text-xs font-mono text-primary placeholder:text-secondary focus:outline-none focus:ring-2 focus:ring-saturn-500"
        />
      </div>
      ...
      {parsedData && (
        <div className="bg-accent/20 border border-border/80 rounded-xl p-4 space-y-3 animate-in fade-in">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-border/60 pb-3">
            ...
            >
              <FileCode className="w-3.5 h-3.5 text-saturn-500" />
            ...
            {parsedData.services.map((svc, idx) => (
              <div key={idx} className="text-xs font-mono space-y-1">
                <div className="flex items-center gap-2 text-primary font-bold">
                  <Layers className="w-3.5 h-3.5 text-saturn-500" />
                  <span>{svc.name}</span>
          ...
          <div className="pt-2 flex justify-end">
            <button
              ...
              disabled={installing}
              className="px-4 py-2 bg-saturn-500 hover:bg-saturn-600 text-white rounded-xl text-xs font-semibold flex items-center gap-2 transition-all shadow-md shadow-saturn-500/20 active:scale-95 disabled:opacity-50"
            >
