      ...
      <div className="bg-card border border-border rounded-2xl w-full max-w-3xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden relative" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border/80 bg-background/50">
          <div className="flex items-center gap-2.5">
            <div className="p-2 rounded-xl bg-orbit-500/15 text-orbit-400 border border-orbit-500/20"><Terminal className="w-5 h-5" /></div>
            <div>
              <h2 className="text-base sm:text-lg font-bold text-primary flex items-center gap-2">
                ...
                {parsedData && <span className="text-xs px-2.5 py-0.5 rounded-full font-semibold bg-orbit-500/15 text-orbit-700 dark:text-orbit-300 border border-orbit-500/30">{parsedData.input_type === 'docker_run' ? 'Docker Run CLI' : 'Docker Compose YAML'}</span>}
              </h2>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          <div className="space-y-1.5">
            <div className="flex items-center justify-between text-xs font-semibold text-secondary">
              <label htmlFor="docker-input" className="flex items-center gap-1.5"><FileCode className="w-3.5 h-3.5 text-orbit-400" />{t('docker.cmd_or_compose_content', 'Comando Docker Run ou Conteúdo Compose:')}</label>
              {parsing && <span className="text-orbit-400 flex items-center gap-1 font-mono text-[11px] animate-pulse"><RefreshCw className="w-3 h-3 animate-spin" />{t('docker.analyzing_params', 'Analisando parâmetros...')}</span>}
            </div>
