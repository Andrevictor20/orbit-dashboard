          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold text-primary flex items-center gap-2">
                ...
                {parsedData && (
                  <span className="text-xs px-2.5 py-0.5 rounded-full font-semibold bg-orbit-500/15 text-orbit-700 dark:text-orbit-300 border border-orbit-500/30">
          ...
          {parseError && (
            <div className="p-3.5 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-700 dark:text-rose-300 text-xs flex items-start gap-2.5">
              ...
              <div>
                ...
                <span className="text-rose-600 dark:text-rose-200 font-mono text-[11px]">{parseError}</span>
              </div>
            </div>
          )}
                ...
                <div className="bg-background/80 border border-border rounded-xl p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-primary flex items-center gap-1.5 uppercase tracking-wider">
                      <Cpu className="w-3.5 h-3.5 text-orbit-500" />
                              ...
                              <div className="flex items-center gap-1.5 font-mono text-sm">
                                ...
                                <input
                                  ...
                                  className={`w-24 px-2 py-1 rounded-lg text-sm font-bold font-mono outline-none border transition-all ${
                                    hasConflict
                                      ? 'bg-amber-500/10 border-amber-500/60 text-amber-800 dark:text-amber-200 focus:ring-2 focus:ring-amber-400'
                            ...
                            <div className="flex items-center gap-2 flex-wrap sm:justify-end">
                              {pInfo.in_use && (
                                <div className="text-[11px] text-secondary">
                                  {pInfo.in_use_by ? (
                                    <span>Em uso por: <strong className="text-amber-700 dark:text-amber-300">{pInfo.in_use_by}</strong></span>
                                  ) : (
                                    <span className="text-amber-700 dark:text-amber-300 font-semibold">Socket do host ocupado</span>
                                  )}
                                </div>
                              )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="bg-background/80 border border-border rounded-xl p-3.5 flex flex-col justify-between">
                  ...
                  {parsedData.services[0]?.volumes.length ? (
                    ...
                    <span className="text-xs text-secondary italic">Nenhum volume mapeado</span>
                  )}
                </div>
                    ...
                    <div className="space-y-1 max-h-28 overflow-y-auto font-mono text-[11px] text-primary">
                      {Object.entries(parsedData.services[0].environment).map(([k, v], i) => (
                        <div key={i} className="truncate p-1 bg-card rounded border border-border/50">
                          <strong className="text-purple-700 dark:text-purple-300">{k}</strong>={v}
                        </div>
                      ))}
                    </div>
                  ) : (
                    <span className="text-xs text-secondary italic">Nenhuma variável configurada</span>
                  )}
                </div>
              </div>
            </div>
              ...
              <div className="space-y-2 text-xs text-primary bg-background/80 p-3 rounded-xl border border-border">
                {activeConflicts.map((c, i) => (
                  <div key={i} className="flex items-center justify-between font-mono">
                    ...
                    <span className="text-amber-700 dark:text-amber-300 font-bold">{c.in_use_by || 'Em uso'}</span>
                  </div>
                ))}
              </div>
