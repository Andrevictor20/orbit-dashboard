...
export const BatchUpdateModal: React.FC<BatchUpdateModalProps> = ({
  ...
  const [filterMode, setFilterMode] = useState<'outdated' | 'all'>('outdated');
  ...
  // Initialize and filter selectable containers
  ...
  const displayedContainers = useMemo(() => {
    if (filterMode === 'outdated') {
      return outdatedContainers.length > 0 ? outdatedContainers : containers;
    }
    return containers;
  }, [filterMode, outdatedContainers, containers]);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (initialSelectedId) {
        ...
        setFilterMode('all');
      } else if (outdatedContainers.length > 0) {
        setSelectedIds(outdatedContainers.map(c => c.id));
        setFilterMode('outdated');
      } else {
        setSelectedIds([]);
        setFilterMode('all');
      }
    }
  }, [isOpen, initialSelectedId, outdatedContainers]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md animate-fade-in">
      <div
        className="relative w-full max-w-3xl max-h-[90vh] flex flex-col bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden text-slate-100"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800/80 bg-slate-900/90 sticky top-0 z-10">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 id="batch-update-title" className="text-lg font-semibold tracking-tight text-white flex items-center gap-2">
                {t('batch_update_modal.title')}
                {outdatedContainers.length > 0 && !isUpdating && !isCompleted && (
                  <span className="text-xs px-2 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400 font-medium">
                    {outdatedContainers.length} pendente(s)
                  </span>
                )}
              </h2>
          ...
          <button
            onClick={onClose}
            disabled={isUpdating}
            className="p-2 text-slate-400 hover:text-white rounded-lg hover:bg-slate-800/60 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {!isUpdating && !isCompleted ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-950/50 p-2 rounded-xl border border-slate-800/60">
                <div className="flex items-center space-x-1.5">
                  ...
                  >
                    {t('batch_update_modal.filter_outdated_only')} ({outdatedContainers.length})
                  </button>
                  <button
                  ...
                  >
                    {t('batch_update_modal.filter_all_containers')} ({containers.length})
                  </button>
                </div>
                        ...
                        <div className="flex items-center space-x-3.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center space-x-2">
                              ...
                              {hasUpdate && (
                                <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold bg-amber-500/10 border border-amber-500/30 text-amber-300">
            ...
            <div className="space-y-5">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3 min-w-0">
                          {task.state === 'pending' && <Clock className="w-4 h-4 text-slate-500" />}
                          {task.state === 'pulling' && <DownloadCloud className="w-4 h-4 text-blue-400 animate-bounce" />}
                          {task.state === 'recreating' && <RefreshCw className="w-4 h-4 text-blue-400 animate-spin" />}
                          {task.state === 'success' && <CheckCircle2 className="w-4 h-4 text-emerald-400" />}
                          {task.state === 'error' && <AlertCircle className="w-4 h-4 text-red-400" />}

                          <div className="min-w-0">
                            <span className="text-sm font-semibold text-white truncate block">{task.name}</span>
                            <span className="text-xs text-slate-400 font-mono truncate block">{task.image}</span>
                          </div>
                        </div>
                      ...
                      {task.state === 'error' && isExpanded && (
                        <div className="mt-3 p-3 bg-red-950/40 border border-red-500/20 rounded-lg text-xs font-mono text-red-200 space-y-1">
                          <div className="font-semibold text-red-300">{task.error}</div>
                          {task.details && task.details !== task.error && (
                            <div className="text-red-300/80 whitespace-pre-wrap">{task.details}</div>
                          )}
                        </div>
                      )}
                    </div>
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-950 overflow-hidden">
                ...
                {showLogs && (
                  <div className="p-3 max-h-40 overflow-y-auto font-mono text-xs text-slate-300 space-y-1 bg-black/40">
                    {logs.map((log, index) => (
                      <div
                        key={index}
                        className={`${
        ...
        {/* Modal Footer */}
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-800/80 bg-slate-900/90 sticky bottom-0 z-10">
          {!isUpdating && !isCompleted ? (
            <>
              <button
                onClick={onClose}
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-white bg-blue-600 hover:bg-blue-500 rounded-xl transition-all shadow-lg shadow-blue-600/20 disabled:opacity-40 disabled:cursor-not-allowed hover:scale-[1.02] active:scale-[0.98]"
              >
            ...
            <>
              <div className="flex items-center space-x-2">
                {failedCount > 0 && !isUpdating && (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-amber-300 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 rounded-xl transition-colors"
                  >
              ...
              <button
                onClick={onClose}
                disabled={isUpdating}
                className="px-5 py-2.5 text-sm font-semibold text-white bg-slate-800 hover:bg-slate-700 rounded-xl transition-colors disabled:opacity-40"
              >
