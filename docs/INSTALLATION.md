# Guia de Instalação e Implantação - Saturn

Este documento apresenta as instruções detalhadas para implantação do Saturn em ambientes de produção e desenvolvimento, incluindo requisitos de sistema, métodos de execução e configuração de proxies reversos.
...
- **Memória RAM:** Mínimo de 512 MB livre (o processo do Saturn consome habitualmente entre 15 MB e 35 MB de RAM em repouso).
...
O instalador automatizado verifica a arquitetura do processador, instala dependências ausentes (como o Docker Engine em distribuições Debian/Ubuntu), configura o diretório de dados em `/DATA/saturn` (ou `~/saturn`) e inicia o container:
...
Para integrar o Saturn ao seu ambiente de containers existente, utilize a especificação de compose abaixo:
...
Para executar o Saturn nativamente no sistema operacional sem o container Docker:
...
1. **Obtenção do Código-Fonte:**
   ```bash
   git clone https://github.com/Andrevictor20/saturn.git
   cd saturn
   ```
...
O Saturn utiliza conexões HTTP padrão para rotas REST e conexões persistentes WebSocket para telemetria em tempo real e emulação de terminal. Ao posicionar o Saturn atrás de um proxy reverso, a propagação de cabeçalhos de upgrade é obrigatória.
...
server {
    listen 80;
    server_name saturn.seu-dominio.lan;
...
```
Reinicie o servidor para aplicar a configuração. Caso não seja possível alterar os parâmetros de kernel, o Saturn ativará automaticamente o fallback de medição de processos via RSS.
